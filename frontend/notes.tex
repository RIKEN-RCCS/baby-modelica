...
\begin{QFACT} (?) Determination of an outside_connector must respect
the place where the connector is declared, and inner/outer
declarations should be ignored for that.  An outside_connector is
distinguished by the use of a name that a connector $c$ is declared
and connected in a class and appears in the first part of a composite
name such as in $\mathrm{connect}(c,...)$ or
$\mathrm{connect}(c.d,...)$.\end{QFACT}

\subsubsection*{Connection restrictions}

Connect equations are fixed at translation-time.

\begin{qquote} (\specref{9.3}\/) $\bullet$\,The connect-equations (and
...
It is described in \specref{9.4}.  An overdetermined_type/record is a
type/record defining $\mathit{equalityConstraint}(x,y)$.  Connections
works with overdetermined_type/record.  It contains equation_operators
...
Overdetermined equations have cycles in the undirected graph of
overdetermined_types/records as nodes and connections as edges.  A
graph is reduced to a tree by removing some of the edges by replacing
them with weaker contraints.

Equation_operators in Connections are used to specify trees in a graph
of connections.  $\mathit{branch}(m.c.r,m.c.r)$ makes an edge between
