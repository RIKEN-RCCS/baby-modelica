...
the declaring class, and the flow direction is considered reversed (a
value is negated) for that connection.  An opposite is an
...
{cardinality} only appears in equation sections (in assertions).
{cardinality} is a translation-time constant, but the connector
reference is not (array indices can be non-constants).

%%%%%%%%%%%%%%%%

\subsection{Conditional connectors}

%%\subsubsection*{Expressions for conditionals}

A conditional component is a connector and it is used only in
connect_equations.  Connect_equations with disabled components are
considered removed.
...
%%%%
Conditionals are retained after component redeclarations.
%%%%
...
components cannot be used in modifiers of declarations.
