...
taken into account in {connect} equations and $\mathrm{cardinality}$.
...
the use of a connector that is declared and used in the class (i.e.,
the connector appears in the first part of a composite name).  Both
$c$ and $c.d$ are outside_connectors when they appear in
$\mathrm{connect}\,(c,...)$ and $\mathrm{connect}\,(c.d,...)$ in the
class declaring $c$.\end{QFACT}
...
empty until (even) a declared component is included by using it
explicitly.
...
$\mathrm{cardinality}$ appears only in equation sections (in
assertions).  Equations including $\mathrm{cardinality}$
(if-equations) are fixed at translation-time.
