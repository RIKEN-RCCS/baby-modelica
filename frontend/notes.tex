...
A non-constant variable (instance) reference does not have any
prefixes of classes nor a preceding dot.

A class reference may be with a preceding dot, which refers to the
unnamed root.  Or, a class reference may follow a variable reference,
in that case classes are considered as packages.
...
indexing.  These are the only predefined_variables.  $\mathit{time}$
is a variable, and thus, not defined in the unnamed-enclosing-class
(it is a package).\end{QRULE}
%%%%
Thus, $\mathit{time}$ cannot have a preceding dot.
\begin{quote}
