...
A non-constant variable reference does not have any prefixes of
classes nor a preceding dot.  A class reference may be with a
preceding dot, where the empty name refers to the package-root.  A
class may be referred to via a variable reference, and in that case,
the class is considered as a package.

There is a single namespace for classes and variables.
%%%%
\begin{QRULE}\label{mixed-class-instance-reference} It is legal to
refer to a class via a variable reference as $x.P$, but the class is
treated as a package.  Thus, it is legal to refer to a constant via a
variable reference as $x.P.c$, but, it is illegal to refer to a class
via a variable reference in a declaration as (illegal)
$x.C~v$.\end{QRULE}

\begin{QFACT} An enclosing class is considered as a package in the
nesting of class definitions, and the elements of an enclosing class
only includes the constants but the variables are ignored.\end{QFACT}
%%%%
See Rule~\ref{model-nesting} for the binary roles of a class.

\begin{QFACT} Array indexing is a part of a composite name.  Array
indexing to general expressions is ungrammatical.\end{QFACT}
%%%%
For example, (illegal) $\{1,2,3\}[1]$ is not allowed.

\subsubsection{Predefined_variables}

\begin{QRULE}\label{time-and-end} A variable $\mathit{time}$ is
visible everywhere.  Variables $\mathit{end}$ are visible in array
indexing.  These are the only predefined_variables.\end{QRULE}

$\mathit{time}$ is a variable and not defined in the package-root (it
is a package), Thus, $\mathit{time}$ cannot have a preceding dot.
%%%%
\begin{quote}
...
base_classes.  It equivalent to
"$\mathbf{import}\;N_i=\mathit{package}.N_i$" for $N_i$ in the
package.  The package part can not be empty in this case.
...
classes to the present class.  \baseimport.  \noimport.  The
...
\begin{QRULE}\label{uniqueness-after-extension} It is illegal if a
base class $A$ of $\mathbf{extends}\;A$ is defined in the class $A$
itself or in its base classes.  This can be interpreted as a lookup of
$A$ just ignores its extends-clauses.\end{QRULE}
...
%%%%
This may be by the requirement of the uniqueness of a class name
resolution.  When another class $A$ were defined in $A$ in the code
...
For more examples of skipping bases of the present class in looking up
of a base class.  It applies to a lookup of both the first part and
the remaining parts of a composite name.
%%%%
\begin{quote}
\begin{lstlisting}[aboveskip=-\baselineskip]
/*ILLEGAL*/
...
Note that some_implementations accept the first case with warnings
($M0$), and will accept the both cases ($M0$ and $M1$) when the
extends-clauses are swapped, i.e., when $\mathbf{extends}\;A$ comes
early.
...
The_specification states that inheritance is unified in case a class
extends a common base_class through distinct intermediate
base_classes.  Thus, mutual dependence might be allowed by unifying
the base classes.
...
allowed only if the modifiers to it are identical.  It is illegal if
the modifiers differ.\end{QRULE}

\begin{qquote}\label{quo:unifying-inherited-elements}
(\specref{5.6.1.4}, in the paragraph {\it{}The inherited contents of
the element\/}) At the end, the current instance is checked whether
their children with the same name are identical and only the first one
of them is kept.  It is an error if they are not identical. (* A
"child" means a component. *)\end{qquote}
%%%%
It requires unifying inherited elements.  Classes and variables
multiply inherited are unified.

\paragraph {Remark}

It is ambiguous what identical means.  Textual equality may not be
sufficient with class modifications.  Consider for example,
$\mathbf{extends}\;A(x=a)$ and $\mathbf{extends}\;A(x=b)$ with $a=0$
and $b=0$.
...
\begin{qquote} (\specref{4.6}, in the table) An operator record can
only extend from an operator record [as a short class definition, and
not from another specialized class].  It is not legal to extend from
any of its enclosing scopes.\end{qquote}

\subsubsection{Unifying defined classes}

\begin{QRULE}\label{multiple-definitions} It is legal if classes with
the same name are defined multiple times in the main class and its
bases, when the definitions are considered identical.  Identical
classes are a sort of unified.  It is defined that classes are
identical if they are textually identical after possible
modifications.\end{QRULE}
%%%%
For example in $\mathit{Fluid/Interfaces}$, the definitions of
...
$\mathit{Modelica}.\mathit{Fluid}.\mathit{Machines}.\mathit{BaseClasses}$.\\
$\mathit{PartialTwoPort}$ and $\mathit{PartialLumpedVolume}$ are
defined in $\mathit{Modelica}.\mathit{Fluid}.\mathit{Interfaces}$.

\subsubsection{Restrictions on adding elements}

\begin{QFACT}\label{extending-simple-types} Extending simple_types is
legal either by a short_class_definition or by an extends-clause.
But, adding elements (class/variable) is illegal.\end{QFACT}
...
restriction.  See Rule~\ref{lookup-shortcut}.\end{QRULE}
...
\begin{QRULE}\label{lexical-enclosing}\label{lookup-shortcut} Lexically enclosing
...
%%%%
%% \begin{QRULE}\label{lookup-shortcut} A lookup of a remaining part of a
...
%%%%
In the following example, the model class is $M$.  The outer $P.X$ is
defined in a package $P$ and it cannot have an inner-outer relation.
In this case, a model $P$ works as a package to a model $P.B$.
%%%%
\begin{quote}
\begin{lstlisting}[aboveskip=-\baselineskip]
/*ILLEGAL*/
...
predefined_functions to reduce overdetermined constraints.
...
A function is described in Section~\ref{sec:functions}.
...
See also Section~\ref{sec:class-equality}.  (*CHECK*) Function calls may
...
A function $\mathit{equalityConstraint}$ may be defined in types and
records.  When it is defined, $\mathit{equalityConstraint(...)}=0$ is
added as equations (0 in the RHS is used for an array of zeros).  See
\specref{9.4.1}.

\subsubsection{Record constructors}\label{sec:no-record-constructor}

There is no syntactic construct (expression) to construct records.  A
record need be declared as a variable.
...
\subsubsection{Predefined_functions on arrays}\label{sec:array-size}
...
Some predefined_functions are overloaded.  They include simple_type
...
\subsection{Predefined_operators}\label{sec:predefined-operators}
...
The predefined_operators are defined for the simple_types, although
...
Variables declared with parameter or constant variability do not
change their values during simulation.  Also, some expressions are
required to be translation-time constants.

\subsubsection{Values of constants}

The value of a constant is specified by an initializer.  In contrast,
the value of a parameter may be specified by an initializer, the
$\mathit{start}$ attribute, or an initial_equation.  When specifying
by the $\mathit{start}$ attribute, $\mathit{fixed}=\mathit{true}$ is
required.  When specifying by an initial_equation,
$\mathit{fixed}=\mathit{false}$ is required.  When specifying by an
initial_equation, a $\mathit{start}$ value is ignored.
%%%%
\begin{quote}
\begin{lstlisting}[aboveskip=-\baselineskip]
model M0
    parameter Real x0=0;
    /* Start value with fixed=true: */
    parameter Real x1 (fixed=true,start=10);
    /* Initial equation with fixed=false: */
    parameter Real x2 (fixed=false);
    /* Initial equation with fixed=false, with start value ignored: */
    parameter Real x3 (fixed=false,start=10);
    initial equation
    x2=20;
    x3=30;
end M0;
...
/*ILLEGAL*/
model M1
    /* Start value with fixed=false: */
    parameter Real x0 (fixed=false,start=10);
    /* Initial equation with fixed=true implied: */
    parameter Real x1;
    initial equation
    x1=10;
end M1;
...
For example, in .Modelica.Fluid.Valves.BaseClasses.PartialValve in the
MSL (with some editting),
%%%%
\begin{quote}
\begin{lstlisting}[aboveskip=-\baselineskip]
...
vast set of expressions are allowed for array dimensions.  They
include array indexing and user defined functions, for example.  Note
that we refer here by constants to both constants and parameters
(parameter is a wider set in the_specification).

\begin{qquote} (\specref{3.8.2}\/) a function or operator with
parameter subexpressions is a parameter expression.\end{qquote}

The_specification lists the functions and variables of constant values
which can take non-constant arguments.  See \specref{3.8.1}\/ and
\specref{3.8.2}.
%%%%
\begin{quote}%\spaceskip=2em
\def\w#1 {\makebox[8em][l]{#1} }\raggedright\let~=\enspace
\w change
\w der
\w edge
\w end
...
\w size(A)
\w size(A,~j)
\makebox[12em][l]{Connections.isRoot(A.R)}
\makebox[12em][l]{Connections.rooted(A.R)}
\end{quote}
...
\subsubsection{Predefined_operators}
...
A function is a function in a usual sense.  A function as a class_kind
is described in Section~\ref{sec:class-kinds}.  A function is
syntactically a package, and no instance is created.  It contains
variables that are inputs and outputs, although it is a package.
...
A dimension of an array variable in a function can be a runtime value.
A part of a dimension of a type defined elsewhere is a
translation-time constant.

\begin{qquote} (\specref{12.2}\/) $\bullet$\,The dimension sizes not
declared with (:) of each array result or array local variable [i.e.,
a non-input components] of a function must be either given by the
input formal parameters, or given by constant or parameter
expressions, or by expressions containing combinations of
those...\end{qquote}

\subsubsection{Local variables}

An input/output or local variable declaration accepts general form of
modifiers as well as initializers by =-sign.

\begin{qquote} (\specref{12.2}\/) $\bullet$\,A formal parameter or
local variable may be initialized through a binding (=) of a default
value in its declaration, ...\end{qquote}

Local variables (non-input/output ones) need be declared in the
protected section.

\subsubsection{(?) Restriction}

\begin{qquote} (\specref{12.2}\/) $\bullet$\,A function may not be used
in connections...\end{qquote}
...
enumerations are members of simple_types.  It may be referred to as
...
\subsection{Predefined_variables}
...
$\mathit{time}$ has a special scoping rule.
...
Variables $\mathit{end}$ have a special scoping rule.
...
There is no operator to construct records.  See
Section~\ref{sec:no-record-constructor}.
...
\begin{quote}%\widespacing
\def\w#1 {\makebox[14em][l]{#1} }\raggedright\let~=\enspace
\w der(expr)
\w delay(expr,~delayTime,~delayMax)
\w delay(expr,~delayTime)
\w cardinality(c)
\w homotopy(actual,~simplified)
...
\begin{quote}%\widespacing
\def\w#1 {\makebox[10em][l]{#1} }\raggedright\let~=\enspace
\w initial()
\w terminal()
\w noEvent(expr)
\w smooth(p,~expr)
\w sample(start,~interval)
\w pre(y)
\w edge(b)
\w change(v)
\w reinit(x,~expr)
...
\begin{quote}%\widespacing
\def\w#1 {\makebox[14em][l]{#1} }\raggedright\let~=\enspace
\w subSample(u,~factor)
\w superSample(u,~factor)
...
\subsection{Record initializers}

The_syntaxer internally holds both a record initializer and the
decomposed initializers for it.  For $R\;x=i$, it holds both $x=i$ and
$x.c=i.c$, for the components $c$ of a record.
...
The_flattener generates initializers for an array of records as
$R\;r\;[N]=\{r_1,r_2,...\}$ by introducing fresh variables
$R\;r_i\;(\mathit{modifiers})$.  It is because records need be
declared as variables.  See Section~\ref{sec:no-record-constructor}.
