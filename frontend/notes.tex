...
Instantiation of expandable_connectors is delayed, while usual classes
are instantiated as the model hierarchy.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Syntaxing}

{Syntaxing} performs simple transformations to make a flat model.  It
rewrites connect_equations to equations.  In that rewriting, it
expands if-equations and for-equations that include connect_equations.
...
%%%%
For example, consider a case $\mathit{.Modelica}$ extends
$\mathit{.Modelica.Icons.Package}$.  As $\mathit{.Modelica.Icons}$ is
textually defined, it is not necessary to process $\mathit{.Modelica}$
first to take the definition of $\mathit{.Modelica.Icons}$.  Thus, it
avoids a potential cycle in definitions.
...
A connect_equation connects variables including component connectors
declared in the connector.

\subsubsection*{Outside_connectors}

An outside_connector is an aspect of a connector when it is connected
in a declaring class.  It is an internal connection to the connector,
and the flow direction is considered reversed (a value is negated) for
that connection.  An opposite is an inside_connector.  Each connector
is considered separate as the inside/outside ones, but they share the
same variables.

\begin{QFACT} Determination of an outside_connector must respect the
place where the connector is declared, and inner/outer declarations
should be ignored for that.  An outside_connector is distinguished by
the use of a connector that is declared and connected in the class
(the connector appears in the first part of a composite name).  Both
$c$ and $c.d$ are outside_connectors when they are used in
$\mathrm{connect}(c,...)$ or $\mathrm{connect}(c.d,...)$ in the class
declaring $c$.\end{QFACT}
...
\subsubsection*{Stream connectors}

See \specref{15}.

A stream connector contains a single scalar real
{\it{}flow_variable\/} and some real {\it{}stream_variables\/} (which
can be arrays).  The $\mathbf{stream}$ can prefix a record then the
components are considered as stream_variables.

inStream(v) and actualStream(v) take a $\mathbf{stream}$ variable
(which may be an array).

\subsubsection*{Connector components}

A connector may contain simple_types, connectors, records, and
operator_records, but not models.

\subsubsection*{Expandable_connectors}

An expandable_connector is a set of connectors, and it only is
connected to other ones.  All components in an expandable_connector
are implicitly considered as connectors.  A component will be included
for the outside use of an expandable_connector even if it is not
declared in the definition.  Additionally, an expandable_connector is
first considered empty until a declared component is included by using
it explicitly.
...
to connect on it.  It is similar to $\mathbf{record}$, when
$\mathbf{connector}$ is used in a (long class) definition.  Connector
declarations may be implicit in expandable_connectors.

\item[expandable_connector] is $\mathbf{connector}$.

\item[operator_record] contains the same elements as
...
$\mathit{equalityConstraint}(x,y)$ is called
