...
declaring class.  An outside_connector is an internal connection
connected in the declaring class, and the flow direction is considered
reversed (a value is negated) for that connection.  An opposite is an
inside_connector.  A connector is considered separate as the
inside/outside ones, but they share the same variables.  The side is
taken into account in {connect} equations and the
$\mathrm{cardinality}$ function.
...
use of a connector that is declared and used in the class (i.e., the
first part of a composite name is a connector).\end{QFACT}
%%%%
For example, both $c$ and $c.m.d$ are outside_connectors when they
appear in $\mathrm{connect}\,(c,...)$ and
$\mathrm{connect}\,(c.m.d,...)$ in the class declaring $c$, where $m$
is a possibly empty list of components (connectors or non-connectors).

(*CHECK*) It is an outside_connector when a non-connector intervenes
like $c.m.d$.
...
connected to another ones.  A component will be included when it is
used.  All components in an expandable_connector are implicitly
...
A {\it{}stream_connector\/} is a connector cotaining stream_variables.
A stream_connector contains a single scalar real
{\it{}flow_variable\/} and some (arrays of) reals of
{\it{}stream_variables\/}.  The components are considered as
stream_variables for a record.
...
\subsubsection*{cardinality function}

The $\mathrm{cardinality}$ function returns the count of connections
on the inside_connector of an argument.  $\mathrm{cardinality}$ only
appears in equation sections (in assertions).  $\mathrm{cardinality}$
is a translation-time constant (if the connector reference is
determined).
